...
const Products = () => {
  ...
  const handleAddToCart = async (product) => {
    ...
    if (!token) {
      ...
      setPopup({ show: true, type: "warning", message: "Please login to add items to cart!", redirect: "/login" });
      navigate("/login");
      return;
    }
    try {
      ...
      setPopup({ show: true, type: "success", message: `${product.name} added to cart!`, redirect: "/cart" });
      navigate("/cart");
    } catch (err) {
      // alert("Failed to add to cart: " + err.message);
      setPopup({ show: true, type: "error", message: "Failed to add to cart: " + err.message, redirect: null });
    }
  };
    ...
    <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl text-center">
      ...
      <section className="bg-white py-6 sm:py-8 md:py-12">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
              ...
              <div className="mb-4 sm:mb-6">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-2 lg:flex-col lg:gap-y-0">
                  {categories.map((c) => (
                    <label
                      key={c.id}
                      className="flex items-center gap-2 mb-0 lg:mb-2 cursor-pointer text-sm sm:text-base"
                    >
                      <input
                        type="radio"
                        name="category"
          ...
          {/* PRODUCTS AREA */}
          <div
            className={`${filterResults ? "lg:col-span-4" : "lg:col-span-3"} order-2 lg:order-none`}
          >
            {/* TOP BAR */}
                ...
                <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                  ...
                  <div className="flex items-center border rounded-lg overflow-hidden flex-1 sm:flex-none">
                    ...
                    <button
                      ...
                      className="bg-red-600 hover:bg-black text-white px-2 sm:px-3 py-2"
                    >
                      <Search size={15} />
                    </button>
                  </div>
            ...
            {filterResults && (
              <div className="bg-red-50 border border-red-200 rounded-lg px-3 sm:px-4 py-2 sm:py-3 mb-4 sm:mb-6 flex items-center justify-between">
                <p className="text-red-700 text-xs sm:text-sm font-medium">
                  Showing {products.length} batteries matched from Quick Battery
                  Finder
                </p>
              </div>
            )}
            ...
            {!loading && error && (
              <div className="text-center py-12 sm:py-16 md:py-20">
                ...
                <button
                  ...
                  className="mt-4 px-4 sm:px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-black text-sm sm:text-base"
                >
                  Retry
                </button>
              </div>
            ...
            {!loading && !error && products.length === 0 && (
              <div className="text-center py-12 sm:py-16 md:py-20">
                <p className="text-gray-500 text-base sm:text-lg">
                  No batteries found for your selection.
                </p>
                {filterResults && (
                  <button
                    onClick={clearFilterResults}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
                {products.map((product) => (
                  ...
                  >
                    ...
                    <p className="text-red-600 font-bold mt-1 sm:mt-2 text-sm sm:text-base">
                      ₹{product.price}
                    </p>
                    <button
...
{!loading && !error && products.length > 0 && (
  <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
    {products.map((product) => (
      <div
        key={product.id}
        className="border border-gray-200 rounded-xl p-3 sm:p-4 hover:shadow-lg transition relative"
      >
        {/* Exchange Badge */}

...
const StatsSection = () => {
  ...
  return (
    <section ref={sectionRef} className="relative w-full py-16 bg-black">
      <div
        className="absolute inset-0 bg-cover bg-center opacity-40"
        style={{ backgroundImage: `url(/src/assets/hero1.webp)` }}
      />
      <div className="relative max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((item, index) => (
          <div
          ...
          >
            <div className="flex items-center gap-4">
              <div className="text-2xl">{item.icon}</div>
              <h2 className="text-3xl font-bold">
                <Counter target={item.value} started={started} />
              </h2>
            </div>
            <div className="mt-4 bg-red-600 inline-block px-4 py-2 text-sm font-semibold">
              {item.label.toUpperCase()}
